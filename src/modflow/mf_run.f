...
C     Specify which modules are to be used
      ...
      USE GWFUPWMODULE, only:HKUPW,SC2UPW
      USE PCGMODULE
      USE SIPMODULE
      USE DE4MODULE
      USE GMGMODULE
...
*     Declare variables
     ...
     &                            IGRID)    
      ...
      !update K and Sy, if needed
      if(ksy_update) then
      open(229965,file='swatmf_K_Sy_changed')
      if(day_total.eq.update_times(ksy_update_num)) then
        do i=1,nrow
          do j=1,ncol
            do k=1,nlay
              HKUPW(j,i,1) = k_update(i,j,ksy_update_num)
              SC2UPW(j,i,1) = sy_update(i,j,ksy_update_num)
              SC2UPW(j,i,1) = SC2UPW(j,i,k) * DELR(j) * DELC(i)
            enddo
          enddo
        enddo
        ksy_update_num = ksy_update_num + 1
        write(229965,*) ksy_update_num,day_total
      endif
      endif
