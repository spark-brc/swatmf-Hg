...
!     define local variables
     ...
     &     basin_area,sub_area,
     &     no3_gw_loading,p_gw_loading,hg_gw_loading,
     &     sub_gw_sw,sub_sw_gw,
     &     sub_gw_sw_no3,sub_sw_gw_no3,sub_drn_sw_no3,
     &     sub_gw_sw_p,sub_sw_gw_p,sub_drn_sw_p,
     &     sub_gw_sw_hg,sub_sw_gw_hg,sub_drn_sw_hg,
     &     thickness,cell_area,
     ...
     &     storativity,spec_storage
      ...
      real Hg_diss_total,fract1,fract2,fract3,sub_Hg0,sub_Hg2,sub_MeHg

      !rtb drain (DRAIN package --> flow to SWAT streams)
      real sub_drn(msub),sub_drn_no3(msub),sub_drn_p(msub),
     &     sub_drn_hg(msub)
      real drnrate(1),no3_drn_loading,p_drn_loading,hg_drn_loading
     ...
     &   day_total.eq.outswatmf(swatmf_out_ctr)) then
        ...
        if(out_RT_gwsw.eq.1) then
        write(30015,*) !skip line, to start values for current day
        write(30015,*) 'Day:',day_total
        write(30015,*) 'Daily GW/SW Hg Exchange for each River Cell'
        endif
        if(out_SWATNP_gwsw.eq.1) then
        write(30016,*) !skip line, to start values for current day
        write(30016,*) 'Day:',day_total
        write(30016,*) 'Daily GW/SW Hg Exchange for each Subbasin'
        write(30016,*) 'Subbasin, RIVER pkg, DRAIN pkg'
        endif
      endif
     ...
     &             day_total.eq.outswatmf(swatmf_out_ctr)) then
        ...
        if(rt_active.eq.1) then
          ...
          hg_gw_loading = rt_rivmass(i,3) !grams of dissolved Hg, from MODFLOW river cell
          hg_gw_loading = hg_gw_loading / 1000. !kg of Hg
          if(day_total.eq.outswatmf(swatmf_out_ctr)) then
            if(out_RT_gwsw.eq.1) then
              write(30007,*) IL,IR,IC,no3_gw_loading
              write(30011,*) IL,IR,IC,p_gw_loading
              write(30015,*) IL,IR,IC,hg_gw_loading
            endif
          endif
        endif
     ...
     &                                   day_count_yr
          ...
          if(rt_active.eq.1) then
            ...
            loading = hg_gw_loading * (-1) !positive now signifies loading to the stream
            loading = loading * (grid2riv_len(m,j)/grid_rivlen)
            sub_gwhg(subID) = sub_gwhg(subID) + loading
          endif
          ...
          if(IBOUND(drn_col,drn_row,drn_lay).gt.0) then
            ...
            if(cell_head.gt.drn_elev) then
              ...
              if(rt_active.eq.1) then
                ...
                hg_drn_loading = rt_drnmass(i,3)
                hg_drn_loading = hg_drn_loading / 1000. !g to kg
              endif
              ...
              if(sub_basin.gt.0) then
                ...
                !mercury
                loading = hg_drn_loading * (-1) !positive now signifies loading to the stream
                sub_drn_hg(sub_basin) = sub_drn_hg(sub_basin)+loading
      ...
      do j=1,msub !rtb
        ...
        !mercury (apportion Hg mercury based on relative amounts of 3 species)
        Hg_diss_total = varoute(46,j) + varoute(47,j) + varoute(48,j)
        if(Hg_diss_total.gt.0) then
          fract1 = varoute(46,j) / Hg_diss_total
          fract2 = varoute(47,j) / Hg_diss_total
          fract3 = varoute(48,j) / Hg_diss_total
        else
          fract1 = 0.
          fract2 = 0.
          fract3 = 0.
        endif
        sub_Hg0 = sub_gwhg(j) * fract1
        sub_Hg2 = sub_gwhg(j) * fract2
        sub_MeHg = sub_gwhg(j) * fract3
        varoute(46,j) = varoute(46,j) + sub_Hg0
        varoute(47,j) = varoute(47,j) + sub_Hg2
        varoute(48,j) = varoute(48,j) + sub_MeHg
          ...
          if(rt_active.eq.1) then
            if(day_total.eq.outswatmf(swatmf_out_ctr)) then
              if(out_SWATNP_gwsw.eq.1) then
                ...
                write(30016,*) j,sub_gwhg(j),sub_drn_hg(j)
              endif
            endif
          endif
      ...
      do j=1,msub
        ...
        sub_gw_sw_hg= sub_gw_sw_hg / sub_km(j) !kg/km2
        sub_gw_sw_hg = sub_gw_sw_hg / 100. !kg/ha
        sub_drn_sw_hg = sub_drn_sw_hg / sub_km(j) !kg/km2
        sub_drn_sw_hg = sub_drn_sw_hg / 100. !kg/ha
