...
!!    Flags for writing output data (SWAT, MODFLOW, RT3D)
      integer :: out_SWAT_recharge,out_MF_recharge,
     &           out_SWAT_channel,out_MF_channel,
     &           out_MODFLOW_gwsw,out_SWAT_gwsw
      integer :: out_RT_gwsw,out_RT_rech,out_SWATNP_gwsw,out_SWATNP_rech
...
!!    pumping rates in the WELL package
      ...
      !rtb soheil (for Ogallala Aquifer modeling)
      logical :: ksy_update
      integer :: num_updates,ksy_update_num
      integer, dimension(:), allocatable :: update_times
      real, dimension(:,:,:), allocatable :: k_update,sy_update
      
      !rtb outside recharge (for HRUs outside the model domain that contribute recharge to MODFLOW cells)
      logical :: outside_recharge
      integer :: num_outside_hrus
      integer, dimension(:), allocatable :: outside_hrus,
     &                                      outside_rows,outside_cols
