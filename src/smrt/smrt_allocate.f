...
!     Allocate the size of disaggregated hru variables before MODFLOW and RT3D runs
      ...
      allocate(rchrg_hg_conc_dhru(dhru))
...
10    allocate(smrt_RECH_tot_mo(ncol,nrow))
      ...
      if(rt_active.eq.1) then
        allocate(smrt_csolute_tot_mo(ncol,nrow,nlay,3))
        allocate(smrt_csolute_avg_mo(ncol,nrow,nlay,3))
        allocate(smrt_csolute_tot_yr(ncol,nrow,nlay,3))
        allocate(smrt_csolute_avg_yr(ncol,nrow,nlay,3))
      endif
