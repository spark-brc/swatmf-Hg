...
!!    rchrg_hg_conc_dhru(:)   |mg/L                |concentration of mercury in recharge water
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!!
!!    ~ ~ ~ Local Definitions ~ ~ ~
!!    name          |units                 |definition
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
     ...
     &               rchrg_no3_conc,rchrg_p_conc,day_total,Hg_perc,
     &               rchrg_hg,rchrg_hg_conc,perchg,varoute
     ...
     &     hru_area_m2,rch_volume
      ...
      !compute nitrate, phosphorus, and mercury in recharge water for current day
      do n=1,nhru
     ...
     &                for day',iida,'year',iyr
      ...
      call smrt_hru2dhru(rchrg_hg_conc, rchrg_hg_conc_dhru)
      ...
      do i=1,nrow
        ...
        enddo
      enddo
      call smrt_dhru2rtgrid3D(rchrg_hg_conc_dhru, conc_recharge)
      do i=1,nrow
        do j=1,ncol
          CRCH(j,i,3) = conc_recharge(j,i) !hg concentration mapped to the 3rd index of CRCH
        enddo
      enddo
     ...
     &               iida,'year',iyr
      ...
      enddo
      write(30013,*)
      write(30017,*) 'Hg recharge (mg/L) for each cell for day',
     &               iida,'year',iyr
      do i=1,nrow
     ...
     &                     (disolvp(subbasin_id) * grid2riv_len(i,j))
          ...
          riv_Hg_mass = varoute(46,subbasin_id) + !dissolved Hg0 (mg)
     &                  varoute(47,subbasin_id) + !dissolved Hg2+ (mg)
     &                  varoute(48,subbasin_id)   !dissolved MeHg (mg)
          riv_volume = varoute(2,subbasin_id) * 1000. !m3 --> L
          rivHg = rivHg + ((riv_Hg_mass/riv_volume)*grid2riv_len(i,j)) !mg/L
        enddo
        ...
        do j=1,MXRIVR
          ...
          if(mf_gridID.eq.grid2riv_gridID(i)) then
            ...
            rt_criv(j,3) = rivHg       !note that mercury is hard-coded to the 3rd entry for RT3D
          endif
        enddo
