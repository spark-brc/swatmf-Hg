...
*     Read whether or not there are constant concentration cells
      read(INSSM,*) header
      read(INSSM,*) rt_ncnh,rt_nwel,rt_ndrn,rt_nriv,rt_nghb
...
*     Allocate (dynamically) arrays for the SSM package
      ...
      !well concentration array (to be filled up by values in SSM  input file)
      allocate(rt_cwell(maxwel,ncomp))
      rt_cwell = 0.
...
C     Point source/sinks of specified concentration
      ...
      !constant head
      if(rt_ncnh.gt.0) then
      
      endif
      
      !wells (injection)
      if(rt_nwel.gt.0) then
        read(INSSM,*) header
	  do num=1,rt_nwel
          !read in: layer, row, column, concentration
          read(INSSM,*) kk,ii,jj,(dum_array(index),index=1,mcomp)
          index = 1
          do k=1,ncomp
            if (phase(k).EQ.1) then !Specie is mobile
              rt_cwell(num,k) = dum_array(index)
              index = index + 1
            else
              rt_cwell(num,k) = 0
            endif
      ...
      !drain
      if(rt_ndrn.gt.0) then
      
      endif
      
      !river
      if(rt_nriv.gt.0) then
      
      endif
      
      !general head boundary
      if(rt_nghb.gt.0) then
...
C     Point Sink/Source Terms (CONST-HEAD, WELL, DRAIN, RIVER & GHB)  -----------------------------
      ...
      do num=1,rt_nwel !loop through the wells
        ...
        i = WELL(2,num)
        j = WELL(3,num)
        qss = WELL(4,num) !flow rate (pumping rate)
        ctmp = rt_cwell(num,icomp) !concentration specified in the SSM input file
