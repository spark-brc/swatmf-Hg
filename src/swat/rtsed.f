...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: depdeg, dot, outfract, deg,por_sed
...
!!  from a small channel than a larger channel. Modification made by Balaji Narasimhan
	ch_a=ch_w(2,jrch)*ch_l2(jrch)*1000. !m2
	  ...
	  if (deg >= depch(jrch)) then
        ...
        dep = 0.
        sed_dep_rto = 0  !total sediment deposition amount in metric tons. jaehak for Mercury modeling.
        sed_deposit = - (deg1 + deg2) 
	  
        if(deg1>0)then
        	deg_z(1)=deg1/(sub_bd(jrch)*ch_a) !erosion depth, m
        else
        	deg_z(1)=0.
        endif
        if(deg2>0)then
        	deg_z(2)=deg2/(sub_bd(jrch)*ch_a) !erosion depth, m
        else
        	deg_z(2)=0.
        endif
	...
	! sediment deposited
	  ...
	  deg2 = 0.
        ...
        sed_dep_rto = min(0.9, dep / sedin) !total sediment deposition amount in metric tons. jaehak for Mercury modeling.
        sed_deposit = dep
	  
	  if(1.-sed_depth2(jrch)>dep/(sub_bd(jrch)*ch_a))then
		dep_z(2)=dep/(sub_bd(jrch)*ch_a) !m  !deposition in the  river bed thickness, m
		dep_z(1)=0. 
	  else 
		if (dep/(sub_bd(jrch)*ch_a)<1.-sed_depth2(jrch)) then
			dep_z(1)=0. !!change in the sediment deposit layer on the river bed, m
		    dep_z(2)=max(0.,dep/(sub_bd(jrch)*ch_a))
		else
			dep_z(2)=max(0.,1.-sed_depth2(jrch)) !!change in the sediment deposit layer on the river bed, m
		    dep_z(1)=max(0.,dep/(sub_bd(jrch)*ch_a-dep_z(2)))
		endif
	  endif
	  
	endif

	!sed_wt(jrch) = max(0.01,sed_wt(jrch) + sed_deposit) !tones sediment in 20cm
