...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !beginning of the day surface residue amount Jaehak for mercury 2017
      rsdinit(j) = sol_rsd(1,j)
...
!        if (pot_vol(j) < 1.e-6) call etact
     ...
     &    smx(j)) - precipday + qday + latq(j) + sepbtm(j) + qtile
        ...
        ! update ground cover for the day for Mercury modeling Jaehak 2017
        frac_groundcover(j) = 1.-Exp(-ext_coef(idplt(j)) *
     &                                        (laiday(j) + .05)) 
        !! check for dormancy
        ...
        hru_et_yr(j) = hru_et_yr(j) + etday !track for annual actual ET for each HRU
...
!!    if 1)current is septic hru and 2)  soil temperature is above zero
	  if (isep_opt(j)/=0.and.iyr>=isep_iyr(j)) then
	   if (sol_tmp(i_sep(j),j) > 0.) call biozone     
        ...
        call gwmod
        call gwmod_deep
        if(mf_active)call smrt_modifySWATgw !rtb MODFLOW
        ...
        !rtb - riyana
        !add nitrogen from extra sources (feed lots, urban areas, etc.)
        if(nit_inputs) then
         do kk=1,num_inputs  
           sol_no3(1,j) = sol_no3(1,j) + (nit_mass(j,kk)/hru_ha(j)/365.)  !kg/ha for the current day
         enddo
		endif
