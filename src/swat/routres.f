...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: resno2c, resnh3c,sedre,tothg
      real :: hgdi(3),hgpi(3),hgdo(3),hgpo(3),hgds(6),hgps(3)
...
!!    ires_code = 1 turn off reservoirs for santhi
  ...
  !!      call res
        ...
        !! calculate Mercury balance Jaehak 2017
        call mercury_res
        ...
        if (curyr > nyskip) then
          ...
          resoutm(6,jres) = resoutm(6,jres) + res_vol(jres) !reservoir storage, m3, modified for Hg modeling Jaehak
          resoutm(7,jres) = resoutm(7,jres) +res_sed(jres)*res_vol(jres)   !sediment in the reservoir, tons, modified for Hg modeling Jaehak
          ...
          resoutm(42,jres) = resoutm(42,jres) + varoute(46,inum2) !dissolved Hg0 in inflow mg
          resoutm(43,jres) = resoutm(43,jres) + varoute(47,inum2) !dissolved Hg2+ in inflow mg
          resoutm(44,jres) = resoutm(44,jres) + varoute(48,inum2) !dissolved MeHg in inflow mg
          
          resoutm(45,jres) = resoutm(45,jres) + varoute(43,inum2) !particulate Hg0 in inflow mg
          resoutm(46,jres) = resoutm(46,jres) + varoute(44,inum2) !particulate Hg2+ in inflow mg
          resoutm(47,jres) = resoutm(47,jres) + varoute(45,inum2) !particulate MeHg in inflow mg

          resoutm(48,jres) = resoutm(48,jres) + varoute(46,ihout) !dissolved Hg0 in outflow mg
          resoutm(49,jres) = resoutm(49,jres) + varoute(47,ihout) !dissolved Hg2+ in outflow mg
          resoutm(50,jres) = resoutm(50,jres) + varoute(48,ihout) !dissolved MeHg in outflow mg
          
          resoutm(51,jres) = resoutm(51,jres) + varoute(43,ihout) !particulate Hg0 in outflow mg
          resoutm(52,jres) = resoutm(52,jres) + varoute(44,ihout) !particulate Hg2+ in outflow mg
          resoutm(53,jres) = resoutm(53,jres) + varoute(45,ihout) !particulate MeHg in outflow mg

          resoutm(54,jres) = resoutm(54,jres) + Hg_resstor(1,1,jres,1)  !dissolved Hg0 stored in reservoir mg
          resoutm(55,jres) = resoutm(55,jres) + Hg_resstor(1,2,jres,1)  !dissolved Hg2+ stored in reservoir mg
          resoutm(56,jres) = resoutm(56,jres) + Hg_resstor(1,3,jres,1)  !dissolved MeHg stored in reservoir mg
          
          resoutm(57,jres) = resoutm(57,jres) + Hg_resstor(2,1,jres,1)  !particulate Hg0 stored in reservoir mg
          resoutm(58,jres) = resoutm(58,jres) + Hg_resstor(2,2,jres,1)  !particulate Hg2+ stored in reservoir mg
          resoutm(59,jres) = resoutm(59,jres) + Hg_resstor(2,3,jres,1)  !particulate MeHg stored in reservoir mg
     ...
     &    lkspst_conc(jres),iyr
        if(resflwi>0.1) then
            hgdi(1) = varoute(46,inum2)                 !dissolved Hg in inflow, mg
            hgdi(2) = varoute(47,inum2)                 !dissolved Hg in inflow, mg
            hgdi(3) = varoute(48,inum2)                 !dissolved Hg in inflow, mg
        else
            hgdi(1:3) = 0.
        endif
        if(ressedi>0.01) then
            hgpi(1) = varoute(43,inum2)            !particulate Hg in inflow, mg
            hgpi(2) = varoute(44,inum2)            !particulate Hg in inflow, mg
            hgpi(3) = varoute(45,inum2)            !particulate Hg in inflow, mg
        else
            hgpi(1:3) = 0.
        endif 
        if(resflwo>0.1) then
            hgdo(1) = varoute(46,ihout)                  !dissolved Hg in outflow, mg
            hgdo(2) = varoute(47,ihout)                  !dissolved Hg in outflow, mg
            hgdo(3) = varoute(48,ihout)                  !dissolved Hg in outflow, mg
        ELSE
            hgdo(1:3) = 0.
        endif
        if(ressedo>0.01) then
            hgpo(1) = varoute(43,ihout)            !particulate Hg in outflow, mg
            hgpo(2) = varoute(44,ihout)            !particulate Hg in outflow, mg
            hgpo(3) = varoute(45,ihout)            !particulate Hg in outflow, mg
        else
            hgpo(1:3) = 0.
        endif
        if(res_vol(jres)>0.1) then
          !hg storage in water
            tothg = Hg_resstor(1,1,jres,1)+Hg_resstor(1,2,jres,1)+
     &           Hg_resstor(1,3,jres,1)+Hg_resstor(2,1,jres,1)+
     &           Hg_resstor(2,2,jres,1)+Hg_resstor(2,3,jres,1)
            hgds(1) = tothg       !THg in stored water, mg
            tothg = Hg_resstor(1,3,jres,1)+Hg_resstor(2,3,jres,1)
            hgds(2) = tothg       !MeHg in stored water, mg
          !hg storage in sed layer 1
            tothg = Hg_resstor(1,1,jres,2)+Hg_resstor(1,2,jres,2)+
     &           Hg_resstor(1,3,jres,2)+Hg_resstor(2,1,jres,2)+
     &           Hg_resstor(2,2,jres,2)+Hg_resstor(2,3,jres,2)
            hgds(3) = tothg       !THg, mg
            
            tothg = Hg_resstor(1,3,jres,2)+Hg_resstor(2,3,jres,2)
            hgds(4) = tothg       !MeHg, mg

           !hg storage in sed layer 2
            tothg = Hg_resstor(1,1,jres,3)+Hg_resstor(1,2,jres,3)+
     &           Hg_resstor(1,3,jres,3)+Hg_resstor(2,1,jres,3)+
     &           Hg_resstor(2,2,jres,3)+Hg_resstor(2,3,jres,3)
            hgds(5) = tothg       !THg, mg
            
            tothg = Hg_resstor(1,3,jres,3)+Hg_resstor(2,3,jres,3)
            hgds(6) = tothg       !MeHg,mg
        else
            hgds(1:2) = 0.
        endif
         
        if(resflwi<0.001) hgdi=0.
        if(ressedi<0.001) hgpi=0.
        if(resflwo<0.001) hgdo=0.
        if(ressedo<0.001) hgpo=0.
  !      if(res_vol(jres)<0.001) hgds=0.
        if(sedre<0.001) hgps=0.
       
        write (7746002,5000) iyr, iida, 
     &     (hgdi(k), k=1,3),        !dissolved Hg in inflow, mg
     &     (hgpi(k), k=1,3),         !particulate Hg in inflow, mg
     &     (hgdo(k), k=1,3),         !dissolved Hg in outflow, mg
     &     (hgpo(k), k=1,3),         !particulate Hg in outflow, mg
     &     hgds(1), hgds(2)        !THg, MeHg stored, mg
!     &     (hgds(k), k=1,3),        !dissolved Hg in stored, mg
!     &     (hgps(k), k=1,3)         !particulate Hg in stored water, mg
