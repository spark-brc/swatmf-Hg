...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      implicit none
      integer jj
      prog = "SWAT Mar 23 2022    VER 2012/Rev 636_smrt_hg"
      write (*,1000)
 1000 format(1x,"               SWAT2012               ",/,             
     &          "               Rev. 636_smrt          ",/,             
     &          "      Soil & Water Assessment Tool    ",/,             
     &          " Extended for Mercury transport simulation",/
     &          "               PC Version             ",/,             
     &          " Program reading from file.cio . . . executing",/)

      
      !!Output file for Mercury simulation
      open(7746118,file="output-mercury.sub")
      open(7746000,file="output-mercury.hru")
      open(7746001,file="output-mercury.rch")
      open(7746002,file="output-mercury.res")
      open(7746003,file="qhg.dat")
	read(7746003,*)
...
!! process input
      ...
      !rtb modflow
      open(445566,file='hru_yield_irrig')
      write(445566,*) 'Annual Crop Yield (metric ton / ha) for each HRU'
      write(445566,*) 'HRU,sub,Area(ha),Year,Crop,Yield,Biomass,Precip,
     &                 SWirrig(mm),GWirrig(mm),ActualET(mm)'
      !open(445567,file='irrigation_switch') 
      !read(445567,*) irrig_switch
      irrig_switch = 1
     
      !rtb riyana - source of additional nitrogen (kg/yr for each HRU)
      inquire(file='nitrogen_inputs',exist=nit_inputs)
      if(nit_inputs) then
        in_nit_inputs = 225533
        open(in_nit_inputs,file='nitrogen_inputs')
        read(in_nit_inputs,*)
        read(in_nit_inputs,*) num_inputs
        allocate(nit_mass(mhru,num_inputs))
        nit_mass = 0.
        read(in_nit_inputs,*)
        do in_index=1,mhru
          read(in_nit_inputs,*) in_hru,
     &												 (nit_mass(in_index,jj),jj=1,num_inputs)
        enddo
      endif
