...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real :: xx
      integer :: eof
      real, dimension(2,3) :: yy
     
      eof = 0
      !Hg_soil_layer(:,:,16:25,:) = Hg_soil_layer(:,:,16:25,:)*0.5
      !Hg_soil_layer(:,:,31:37,:) = Hg_soil_layer(:,:,31:37,:)*1.0
      !Hg_soil_layer(:,:,124:134,:) = Hg_soil_layer(:,:,124:134,:)*1.4
      !Hg_soil_layer(:,:,312:339,:) = Hg_soil_layer(:,:,312:339,:)*0.7

      !Loop through the years of the simulation
      day_total = 0
      do curyr = 1, nbyr
        
        write (*,1234) curyr,iyr
...
!          if (biomix(j) > .001) call tillmix (j,biomix(j))
     ...
     &                     mat_yrs(idplt(j)))
      ...
      return
 1234 format (1x,' Executing year ', i4,2x,i4)
      end