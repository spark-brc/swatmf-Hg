...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: j, k
      real :: sum,fin,fout,sin,sout,svol,tothg
      real, dimension(3)::hgdi,hgpi,hgdo,hgpo,hgds,hgps
     ...
     &                     (hrupsty(k,1,j), hrupsty(k,2,j), k = 1, npmx)
          ...
          !rtb modflow - write annual crop yield for each hru
          do j=1,nhru
            cropname = cpnm(idplt(j))
            write(445566,6302) j,hru_sub(j),hru_ha(j),iyr,cropname,
     &                         yldanu(j),bio_yrms(j),hruyro(1,j),
     &                         irrig_yr_sw(j),irrig_yr_gw(j),
     &                         hru_et_yr(j)
          enddo
...
!         !! annual write--sediment routing (.sed)
          ...
          do j = 1, nres
            ...
            resouty(42,j) = resouty(42,j) / Real(idlast)
            resouty(43,j) = resouty(43,j) / Real(idlast)
            resouty(44,j) = resouty(44,j) / Real(idlast)
            resouty(45,j) = resouty(45,j) / Real(idlast)
            resouty(46,j) = resouty(46,j) / Real(idlast)
            resouty(47,j) = resouty(47,j) / Real(idlast)
            resouty(48,j) = resouty(48,j) / Real(idlast)
            resouty(49,j) = resouty(49,j) / Real(idlast)
            resouty(50,j) = resouty(50,j) / Real(idlast)
            resouty(51,j) = resouty(51,j) / Real(idlast)
            resouty(52,j) = resouty(52,j) / Real(idlast)
            resouty(53,j) = resouty(53,j) / Real(idlast)
            resouty(54,j) = resouty(54,j) / Real(idlast)
            resouty(55,j) = resouty(55,j) / Real(idlast)
            resouty(56,j) = resouty(56,j) / Real(idlast)
            resouty(57,j) = resouty(57,j) / Real(idlast)
            resouty(58,j) = resouty(58,j) / Real(idlast)
            resouty(59,j) = resouty(59,j) / Real(idlast)

            fin = resouty(1,j) * 86400. !daily inflow, m3
            fout =  resouty(2,j) * 86400. !daily outflow, m3
            sin = resouty(3,j) * 1000. !kg
            sout = resouty(4,j) * 1000.!kg

            if(fin>1.)then
                hgdi(1) = resouty(42,j)/fin                 !dissolved Hg in inflow, ppb
                hgdi(2) = resouty(43,j)/fin                 !dissolved Hg in inflow, ppb
                hgdi(3) = resouty(44,j)/fin                 !dissolved Hg in inflow, ppb
            else
                hgdi(1:3) = 0.
            endif
            if(sin>0.1)then
                hgpi(1) = resouty(45,j)/sin            !particulate Hg in inflow, ng/g
                hgpi(2) = resouty(46,j)/sin            !particulate Hg in inflow, ng/g
                hgpi(3) = resouty(47,j)/sin            !particulate Hg in inflow, ng/g
            else
                hgpi(1:3) = 0.
            endif 
            if(fout>1.)then
                hgdo(1) = resouty(48,j)/fout                  !dissolved Hg in outflow, ppb
                hgdo(2) = resouty(49,j)/fout                  !dissolved Hg in outflow, ppb
                hgdo(3) = resouty(50,j)/fout                  !dissolved Hg in outflow, ppb
            ELSE
                hgdo(1:3) = 0.
            endif
            if(sout>0.1)then
                hgpo(1) = resouty(51,j)/sout            !particulate Hg in outflow, ng/g
                hgpo(2) = resouty(52,j)/sout            !particulate Hg in outflow, ng/g
                hgpo(3) = resouty(53,j)/sout            !particulate Hg in outflow, ng/g
            else
                hgpo(1:3) = 0.
            endif
            if(resouty(6,j)>1.)then
                tothg = resouty(54,j)+resouty(55,j)+resouty(56,j)
     &           +resouty(57,j)+resouty(58,j)+resouty(59,j)
                hgds(1) = tothg/resouty(6,j)       !THg stored in reservoir  ppb
                tothg = resouty(56,j)+resouty(59,j)
                hgds(2) = tothg/resouty(6,j)       !MeHg stored in reservoir  ppb
            else
                hgds(1:3) = 0.
            endif
            !if(resouty(7,j)>0.1)then
            !    hgps(1) = resouty(57,j)/resouty(7,j)               !particulate Hg in stored water, ng/g
            !    hgps(2) = resouty(58,j)/resouty(7,j)               !particulate Hg in stored water, ng/g
            !    hgps(3) = resouty(59,j)/resouty(7,j)               !particulate Hg in stored water, ng/g
            ! else
            !     hgps(1:3) = 0.
            ! endif
            ...
            if(fin<0.001) hgdi=0.
            if(sin<0.001) hgpi=0.
            if(fout<0.001) hgdo=0.
            if(sout<0.001) hgpo=0.
            if(res_vol(j)<0.001) hgds=0.
            if(svol<0.001) hgps=0.
     ...
     &                       (resouty(k,j), k = 6, 16)
              write (7746002,5800) j, iyr,
     &                      (hgdi(k), k=1,3),        !dissolved Hg in inflow, ppb
     &                      (hgpi(k), k=1,3),         !particulate Hg in inflow, ng/g
     &                      (hgdo(k), k=1,3),         !dissolved Hg in outflow, ppb
     &                      (hgpo(k), k=1,3),         !particulate Hg in outflow, ng/g
     &                      (hgds(k), k=1,2)        !Hg in stored water,, ppb
!     &                      (hgds(k), k=1,3),        !dissolved Hg in inflow, ppb
!     &                      (hgps(k), k=1,3)         !particulate Hg in stored water, ng/g
...
!     &                      (hgps(k), k=1,3)         !particulate Hg in stored water, ng/g
 ...
 6301 format (i5,8f8.2,3f12.2,40f8.2)
 6302 format (i8,i8,f12.4,i8,a8,10f12.4)
      end