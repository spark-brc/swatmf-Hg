...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	if (aird(j)>0) then
	  j=j
        if(irrsc(j).eq.1) then !river reach source: store annual surface water irrigation for HRU
          irrig_yr_sw(j) = irrig_yr_sw(j) + aird(j)
        endif
	end if
     ...
     &                        mf_irrigation_swat.eq.0) then
        ...
        if(mf_sub_irrig_flag(inum1).eq.1) then
          ...
          irrig_yr_gw(j) = irrig_yr_gw(j) + aird(j)
