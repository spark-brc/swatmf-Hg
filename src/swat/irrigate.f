      ...
      if(mf_active.eq.1 .and. mf_irrigation_swat.eq.1) then
        ...
        do n=1,ncell_irrigate
          if(jj.eq.mf_sub_irrigate(n,4)) then !found HRU
            ...
            !available groundwater in the pumping cell
            cell_row = mf_sub_irrigate(n,2)
            cell_col = mf_sub_irrigate(n,3)
            cell_lay = mf_sub_irrigate(n,5)
            gw_vol = gw_available(cell_col,cell_row,cell_lay) !m3 
            
            !volume of irrigation water requested
            hru_area = hru_km(jj) * 1000000. !m2
            irrig_vol = (volmm/1000.) * hru_area

            !compare; if requested volume is more than available volume, decrease the depth
            if(irrig_vol.gt.gw_vol) then
              ...
              gw_available(cell_col,cell_row,cell_lay) = 0. !all groundwater taken
            endif

            !set in the irrigation depth array
