...
!!    latq contribution
        ...
        pdvas(80) = hruyro(73,j)
        pdvas(81) = hruyro(74,j)
        pdvas(82) = hruyro(75,j)
        pdvas(83) = hruyro(76,j)
        pdvas(84) = hruyro(77,j)
        pdvas(85) = hruyro(78,j)
        pdvas(86) = hruyro(79,j)
        pdvas(87) = hruyro(80,j)
        pdvas(88) = hruyro(81,j)
        pdvas(89) = hruyro(82,j)
        pdvas(90) = hruyro(83,j) / Real(366 - leapyr)
        pdvas(91) = hruyro(84,j) / Real(366 - leapyr)
        pdvas(92) = hruyro(85,j) / Real(366 - leapyr)
        pdvas(93) = hruyro(86,j) / Real(366 - leapyr)
        pdvas(94) = hruyro(87,j) / Real(366 - leapyr)
        pdvas(95) = hruyro(88,j) / Real(366 - leapyr)
        pdvas(96) = hruyro(89,j) / Real(366 - leapyr)
        pdvas(97) = hruyro(90,j) / Real(366 - leapyr)
        pdvas(98) = hruyro(91,j) / Real(366 - leapyr)
        pdvas(99) = hruyro(92,j) / Real(366 - leapyr)
        pdvas(100) = hruyro(93,j) / Real(366 - leapyr)
        pdvas(101) = hruyro(94,j) / Real(366 - leapyr)
        pdvas(102) = hruyro(95,j) / Real(366 - leapyr)
        pdvas(103) = hruyro(96,j) / Real(366 - leapyr)
        pdvas(104) = hruyro(97,j) / Real(366 - leapyr)
        pdvas(105) = hruyro(98,j)
        pdvas(106) = hruyro(99,j)
        pdvas(107) = hruyro(100,j)
        pdvas(108) = hruyro(101,j)
        pdvas(109) = hruyro(102,j)
        pdvas(110) = hruyro(103,j)
        pdvas(111) = hruyro(104,j)
        pdvas(112) = hruyro(105,j)
        pdvas(113) = hruyro(106,j)
        pdvas(114) = hruyro(107,j)
        pdvas(115) = hruyro(108,j)
        pdvas(116) = hruyro(109,j)
        pdvas(117) = hruyro(110,j)
        pdvas(118) = hruyro(111,j)
        pdvas(119) = hruyro(112,j)
        pdvas(120) = hruyro(113,j) / Real(366 - leapyr)
        pdvas(121) = hruyro(114,j)
        pdvas(122) = hruyro(115,j)
        pdvas(123) = hruyro(116,j)
        pdvas(124) = hruyro(117,j)
 ...
 1000 format (a4,i5,1x,a5,a4,i5,1x,i4,1x,i3,1x,i3,1x,e10.5,119E12.3)

          if (iscen == 1 .and. isproj == 0) then
          write (28,1000) cropname, j, subnum(j), hruno(j), sb,         
     &             iyr, 0,0,hru_km(j), (pdvs(ii), ii = 1, 79)
          write (7746000,1000) cropname, j, subnum(j), hruno(j), sb,         
     &             iyr, 0,0,hru_km(j), (pdvs(ii), ii = 80, itots)
          else if (isproj == 1) then
          write (21,1000) cropname, j, subnum(j), hruno(j),             
     &            sb, iyr, 0,0,hru_km(j), (pdvs(ii), ii = 1, itots)
          else if (iscen == 1 .and. isproj == 2) then
          write (28,2000) cropname, j, subnum(j), hruno(j), sb,         
     &    nmgt(j), iyr, hru_km(j), (pdvs(ii), ii = 1, 79)
          write (7746000,2000) cropname, j, subnum(j), hruno(j), sb,         
     &    nmgt(j), iyr, hru_km(j), (pdvs(ii), ii = 80, itots)
          endif
        else
          if (iscen == 1 .and. isproj == 0) then
          write (28,1001) cropname, j, subnum(j), hruno(j), sb,         
     &            nmgt(j), iyr, hru_km(j), (pdvas(ii), ii = 1, 79)
          write (7746000,1001) cropname, j, subnum(j), hruno(j), sb,         
     &            nmgt(j), iyr, hru_km(j), (pdvas(ii), ii = 80, mhruo)
          else if (isproj == 1) then
          write (21,1001) cropname, j, subnum(j), hruno(j),             
     &           sb, nmgt(j), iyr, hru_km(j), (pdvas(ii), ii = 1, mhruo)
          else if (iscen == 1 .and. isproj == 2) then
          write (28,1001) cropname, j, subnum(j), hruno(j), sb,         
     &    nmgt(j), iyr, hru_km(j), (pdvas(ii), ii = 1, 79), iyr
          write (7746000,1001) cropname, j, subnum(j), hruno(j), sb,         
     &    nmgt(j), iyr, hru_km(j), (pdvas(ii), ii = 80, mhruo)
          endif
        end if
 ...
 2000 format (a4,i5,1x,a5,a4,i5,1x,i4,1x,i4,e10.5,119E12.3)
										 
 1001 format (a4,i7,1x,a5,a4,i5,1x,i4,1x,i4,e10.5,119e12.3)      
      end