...
!       immo = immo + 1
     ...
     &                     (hrupstm(k,1,j), hrupstm(k,2,j), k = 1, npmx)
            ...
            do j = 1, nres
                fin = resoutm(1,j) * 86400. ! inflow, m3
                fout = resoutm(2,j) * 86400. ! outflow, m3
                sin = resoutm(3,j)  !ton
                sout = resoutm(4,j) !ton
              if(fin>1.)then
                  hgdi(1) = resoutm(42,j)/fin                 !dissolved Hg in inflow, ppb
                  hgdi(2) = resoutm(43,j)/fin                 !dissolved Hg in inflow, ppb
                  hgdi(3) = resoutm(44,j)/fin                 !dissolved Hg in inflow, ppb
              else
                  hgdi(1:3) = 0.
                  endif
              if(sin>0.1)then
                  hgpi(1) = resoutm(45,j)/sin            !particulate Hg in inflow, ng/g
                  hgpi(2) = resoutm(46,j)/sin            !particulate Hg in inflow, ng/g
                  hgpi(3) = resoutm(47,j)/sin            !particulate Hg in inflow, ng/g
              else
                  hgpi(1:3) = 0.
                  endif
              if(fout>1.)then
                  hgdo(1) = resoutm(48,j)/fout           !dissolved Hg in outflow, ppb
                  hgdo(2) = resoutm(49,j)/fout           !dissolved Hg in outflow, ppb
                  hgdo(3) = resoutm(50,j)/fout           !dissolved Hg in outflow, ppb
              else
                  hgdo(1:3) = 0.
              endif
              if(sout>0.1)then
                  hgpo(1) = resoutm(51,j)/sout            !particulate Hg in outflow, ng/g
                  hgpo(2) = resoutm(52,j)/sout            !particulate Hg in outflow, ng/g
                  hgpo(3) = resoutm(53,j)/sout            !particulate Hg in outflow, ng/g
              else
                  hgpo(1:3) = 0.
              endif
              !if(resoutm(6,j)>1.)then
              !    hgds(1) = resoutm(54,j)/resoutm(6,j)       !dissolved Hg0 stored in reservoir  ppb
              !    hgds(2) = resoutm(55,j)/resoutm(6,j)       !dissolved Hg2+ stored in reservoir ppb
              !    hgds(3) = resoutm(56,j)/resoutm(6,j)       !dissolved MeHg stored in reservoir ppb
              !else
              !    hgds(1:3) = 0.                           
              !endif                                        
              !if(resoutm(7,j)>0.1)then                             
              !    hgps(1) = resoutm(57,j)/resoutm(7,j)        !particulate Hg0 stored in reservoir ng/g
              !    hgps(2) = resoutm(58,j)/resoutm(7,j)        !particulate Hg2+ stored in reservoir ng/g
              !    hgps(3) = resoutm(59,j)/resoutm(7,j)        !particulate MeHg stored in reservoir ng/g
              !else
              !    hgps(1:3) = 0.
              !endif
              if(resoutm(6,j)>1.)then
                  tothg = resoutm(54,j)+resoutm(55,j)+resoutm(56,j)
     &           +resoutm(57,j)+resoutm(58,j)+resoutm(59,j)
                  hgds(1) = tothg/resoutm(6,j)       !THg stored in reservoir  ppb
                  tothg = resoutm(56,j)+resoutm(59,j)
                  hgds(2) = tothg/resoutm(6,j)       !MeHg stored in reservoir  ppb
              else
                  hgds(1:3) = 0.                           
              endif                                        
...
!     &                      (hgps(k), k=1,3)         !particulate Hg in stored water, ng/g
 ...
 6201 format (i5,8f8.2,3f12.2,40f8.2)
