...
! Mercury Jaehak 2017
      pdvas(80) = Hg_dry_total(1,2,j) !total dissolved Hg2 dry deposition (mg/ha)
      pdvas(81) = Hg_dry_total(2,2,j) !total particulate Hg2+ dry deposition (mg/ha)
      pdvas(82) = Hg_dry_leaf(1,2,j) !leaf dissolved Hg2 dry deposition (mg/ha)
      pdvas(83) = Hg_dry_leaf(2,2,j) !land particulate Hg2+ dry deposition (mg/ha)
      pdvas(84) = Hg_wet_total(1,2,j) !! total dissolved Hg2+ wet deposition, mg/ha
      pdvas(85) = Hg_wet_direct(1,2,j) !! direct land dissolved Hg2+ wet deposition, mg/ha
      pdvas(86) = Hg_wet_thru(1,2,j) ! throughfall dissolved Hg2+ wet deposition, mg/ha
      pdvas(87) = Hg_leaf_fall(1,1,j) !leaf fall of dissolved Hg0 (mg/ha)
      pdvas(88) = Hg_leaf_fall(1,2,j) !leaf fall of dissolved Hg2+ (mg/ha)
      pdvas(89) = Hg_leaf_fall(2,2,j) !leaf fall of particulate Hg2+ (mg/ha)
      pdvas(90) = Hg_leaf(1,1,j,1)+Hg_leaf(1,1,j,2) !leaf storage of dissolved Hg0 (mg/ha)
      pdvas(91) = Hg_leaf(1,2,j,1)+Hg_leaf(1,2,j,2) !leaf storage of dissolved Hg+ (mg/ha)
      pdvas(92) = Hg_leaf(2,2,j,1)+Hg_leaf(2,2,j,2) !leaf storage of particulate Hg2+ (mg/ha)
      pdvas(93) = Hg_residue(1,1,j) !residue storage of dissolved Hg0 (mg/ha)
      pdvas(94) = Hg_residue(1,2,j) !residue storage of dissolved Hg2+ (mg/ha)
      pdvas(95) = Hg_residue(2,2,j) !residue storage of particulate Hg2+ (mg/ha)
      pdvas(96) = Hg_soil(1,1,j) !soil storage of dissolved Hg0 (mg/ha)
      pdvas(97) = Hg_soil(1,2,j) !soil storage of dissolved Hg2+ (mg/ha)
      pdvas(98) = Hg_soil(1,3,j) !soil storage of dissolved MeHg (mg/ha)
      pdvas(99) = Hg_soil(2,1,j) !soil storage of particulate Hg0 (mg/ha)
      pdvas(100) = Hg_soil(2,2,j) !soil storage of particulate Hg2+ (mg/ha)
      pdvas(101) = Hg_soil(2,3,j) !soil storage of particulate MeHg (mg/ha)
      pdvas(102) = Hg_shallst(1,1,j) !shallow aquifer storage of dissolved Hg0 (mg/ha)
      pdvas(103) = Hg_shallst(1,2,j) !shallow aquifer storage of dissolved Hg2+ (mg/ha)
      pdvas(104) = Hg_shallst(1,3,j) !shallow aquifer storage of dissolved MeHg (mg/ha)
      pdvas(105) = Hg_surq(1,j) !dissolved Hg0 in runoff (mg/ha)
      pdvas(106) = Hg_surq(2,j) !dissolved Hg2+ in runoff (mg/ha)
      pdvas(107) = Hg_surq(3,j) !dissolved MeHg in runoff (mg/ha)
      pdvas(108) = Hg_latq(1,j) !dissolved Hg0 in lateral flow (mg/ha)
      pdvas(109) = Hg_latq(2,j) !dissolved Hg2+ in lateral flow (mg/ha)
      pdvas(110) = Hg_latq(3,j) !dissolved MeHg in lateral flow (mg/ha)
      pdvas(111) = Hg_perc(1,j) !dissolved Hg0 percolated to shallow aquifer (mg/ha)
      pdvas(112) = Hg_perc(2,j) !dissolved Hg2+ percolated to shallow aquifer (mg/ha)
      pdvas(113) = Hg_perc(3,j) !dissolved MeHg percolated to shallow aquifer (mg/ha)
      pdvas(114) = Hg_gwq(1,j) !dissolved Hg0 in groundwater return flow (mg/ha)
      pdvas(115) = Hg_gwq(2,j) !dissolved Hg2+ in groundwater return flow (mg/ha)
      pdvas(116) = Hg_gwq(3,j) !dissolved MeHg in groundwater return flow (mg/ha)
      pdvas(117) = Hg_sedyld(1,j) !particulate Hg0 in sediment yield (mg/ha)
      pdvas(118) = Hg_sedyld(2,j) !particulate Hg2+ in sediment yield (mg/ha)
      pdvas(119) = Hg_sedyld(3,j) !particulate MeHg in sediment yield (mg/ha)
      pdvas(120) = Hg_sc(j) !total Hg concentration in soil (ng/kg)
      pdvas(121) = Hg0_vola(j) !Hg volatilization from soil to atmosphere (mg/ha)
      ...
      if (ipdvas(1) > 0) then
          ...
          if (iscen == 1 .and. isproj == 0) then
            if (icalen == 0) then
                write (28,1001) cropname, j, subnum(j), hruno(j), sb, iyr,i_mo, iida, hru_km(j), (pdvs(ii), ii = 1, 79)
																		 
                write (7746000,1001) cropname, j, subnum(j), hruno(j), sb, iyr,i_mo, iida, hru_km(j), (pdvs(ii), ii = 80, itots)
            end if
            
            if (icalen == 1) then
                write (28,1001) cropname, j, subnum(j), hruno(j), sb, iyr, i_mo, icl(iida), hru_km(j), (pdvs(ii), ii = 1, 79)
																		 
                write (7746000,1001) cropname, j, subnum(j), hruno(j), sb, iyr, i_mo, icl(iida), hru_km(j), (pdvs(ii), ii = 80, itots)
            end if
            
1001  format (a4,i5,1x,a5,a4,i5,1x,i4,1x,i3,1x,i3,1x,e10.5,120E12.3)
											   
      
             !!    added for binary files 3/25/09 gsm line below and write (33333
	        if (ia_b == 1) then
																		   
	            write (33333) j, hrugis(j), sb,nmgt(j), iida, hru_km(j), (pdvs(ii), ii = 1, itots)
	        endif
    ...
    1003  format(a4,i5,1x,a5,a4,i5,1x,i4,1x,i2,1x,i2,1x,i4,1x,e10.5,66f10.3,&
        ...
        if (iscen == 1 .and. isproj == 0) then
            if(icalen == 0) then
                write (28,1000) cropname, j, subnum(j), hruno(j),sb,iyr,i_mo, iida, hru_km(j), (pdvas(ii), ii = 1, 79)
                write (7746000,1000) cropname, j, subnum(j), hruno(j),sb,iyr,i_mo, iida, hru_km(j), (pdvas(ii), ii = 80, mhruo)
            end if
            
            if(icalen == 1) then
                write (28,1003) cropname, j, subnum(j), hruno(j),sb,iyr,i_mo, icl(iida), hru_km(j),(pdvas(ii), ii = 1, 79)
                write (7746000,1003) cropname, j, subnum(j), hruno(j),sb,iyr,i_mo, icl(iida), hru_km(j),(pdvas(ii), ii = 80, mhruo)
            end if
...
1000  format (a4,i5,1x,a5,a4,i5,1x,i4,1x,i4,e12.5,120E12.3)
      !!add by zhang
