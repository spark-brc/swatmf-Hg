...
!! Mercury 2017 Jaehak
        pdvar(47) = rchyro(47,j)    !dissolved Hg0 mg  
        pdvar(48) = rchyro(48,j)    !dissolved Hg2+ mg  
        pdvar(49) = rchyro(49,j)    !dissolved MeHg mg  
        pdvar(50) = rchyro(50,j)    !particulate Hg0 mg 
        pdvar(51) = rchyro(51,j)    !particulate Hg2+ mg 
        pdvar(52) = rchyro(52,j)    !particulate MeHg mg  
        pdvar(53) = rchyro(53,j)     !inflow dissolved Hg0 mg  
        pdvar(54) = rchyro(54,j)     !inflow dissolved Hg2+ mg  
        pdvar(55) = rchyro(55,j)     !inflow dissolved MeHg mg  
        pdvar(56) = rchyro(56,j)     !inflow particulate Hg0 mg 
        pdvar(57) = rchyro(57,j)     !inflow particulate Hg2+ mg 
        pdvar(58) = rchyro(58,j)     !inflow particulate MeHg mg 
        pdvar(59) = rchyro(59,j)/ Real(idlast)     !channel stored dissolved Hg0 mg    
        pdvar(60) = rchyro(60,j)/ Real(idlast)     !channel stored dissolved Hg2+ mg     
        pdvar(61) = rchyro(61,j)/ Real(idlast)     !channel stored dissolved MeHg mg      
        pdvar(62) = rchyro(62,j)/ Real(idlast)     !channel stored particulate Hg0 mg     
        pdvar(63) = rchyro(63,j)/ Real(idlast)     !channel stored particulate Hg2+ mg    
        pdvar(64) = rchyro(64,j)/ Real(idlast)     !channel stored particulate MeHg mg     
         if (ipdvar(1) > 0) then
          do ii = 1, itotr
     ...
     &                                    (pdvr(ii), ii = 1, 46)
          write (7746001,5000) j, subgis(j), iyr, rch_dakm(j),pdvr(2),                
     &                                    (pdvr(ii), ii = 47, itotr)
     ...
     &                             (pdvr(ii), ii = 1, 46) 
          write (7746001,6000) j, subgis(j), iyr, rch_dakm(j),pdvr(2),                
     &                             (pdvr(ii), ii = 47, itotr) 
          endif
        else
