...
!! sum HRU results for subbasin 
      ...
      !! subbasin averages: nutrients
        ...
        !! subbasin averages for Mercury Jaehak 2017
        Hg_dry_total_sub(:,:,sb) = Hg_dry_total_sub(:,:,sb) + Hg_dry_total(:,:,j) * hru_fr(j)
        Hg_dry_leaf_sub(:,:,sb) = Hg_dry_leaf_sub(:,:,sb) + Hg_dry_leaf(:,:,j) * hru_fr(j)
        Hg_dry_land_sub(:,:,sb) = Hg_dry_land_sub(:,:,sb) + Hg_dry_land(:,:,j) * hru_fr(j)
        Hg_wet_total_sub(:,:,sb) = Hg_wet_total_sub(:,:,sb) + Hg_wet_total(:,:,j) * hru_fr(j)
        Hg_wet_direct_sub(:,:,sb) = Hg_wet_direct_sub(:,:,sb) + Hg_wet_direct(:,:,j) * hru_fr(j)
        Hg_wet_thru_sub(:,:,sb) = Hg_wet_thru_sub(:,:,sb) + Hg_wet_thru(:,:,j) * hru_fr(j)
        Hg_leaf_sub(:,:,sb) = Hg_leaf_sub(:,:,sb) + (Hg_leaf(:,:,j,1)+Hg_leaf(:,:,j,2)) * hru_fr(j)
        Hg_leaf_fall_sub(:,:,sb) = Hg_leaf_fall_sub(:,:,sb) + Hg_leaf_fall(:,:,j) * hru_fr(j)
        Hg_residue_sub(:,:,sb) = Hg_residue_sub(:,:,sb) + Hg_residue(:,:,j) * hru_fr(j)
        Hg_soil_sub(:,:,sb) = Hg_soil_sub(:,:,sb) + Hg_soil(:,:,j) * hru_fr(j)
        Hg_shallst_sub(:,:,sb) = Hg_shallst_sub(:,:,sb) + Hg_shallst(:,:,j) * hru_fr(j)
        Hg_surq_sub(:,sb) = Hg_surq_sub(:,sb) + Hg_surq(:,j) * hru_fr(j)
        Hg_sedyld_sub(:,sb) = Hg_sedyld_sub(:,sb) + Hg_sedyld(:,j) * hru_fr(j) !mg/ha
        Hg_latq_sub(:,sb) = Hg_latq_sub(:,sb) + Hg_latq(:,j) * hru_fr(j)
        Hg_gwq_sub(:,sb) = Hg_gwq_sub(:,sb) + Hg_gwq(:,j) * hru_fr(j)
        Hg_perc_sub(:,sb) = Hg_perc_sub(:,sb) + Hg_perc(:,j) * hru_fr(j)
        Hg_sc_sub(sb) = Hg_sc_sub(sb) + Hg_sc(j) * hru_fr(j)
        Hg_wyld_sub(:,sb) = Hg_wyld_sub(:,sb) + Hg_wyld(:,j) * hru_fr(j) !mg/ha
        Hg0_vola_sub(sb) = Hg0_vola_sub(sb) + Hg0_vola(j) * hru_fr(j)
...
!        sub_minps(sb) = sub_minps(sb) / sub_fr(sb)
        ...
        !! assign reach loadings for subbasin
         ...
         !Mercury subbasin output Jaehak 2017
         varoute(34,ihout) = Hg_surq_sub(1,sb) * sub_ha     !dissolved Hg0 in runoff (mg)
         varoute(35,ihout) = Hg_surq_sub(2,sb) * sub_ha    !dissolved Hg2+ in runoff (mg)
         varoute(36,ihout) = Hg_surq_sub(3,sb) * sub_ha    !dissolved MeHg in runoff (mg)
         varoute(37,ihout) = Hg_latq_sub(1,sb) * sub_ha    !dissolved Hg0 in lateral flow (mg)
         varoute(38,ihout) = Hg_latq_sub(2,sb) * sub_ha    !dissolved Hg2+ in lateral flow (mg)
         varoute(39,ihout) = Hg_latq_sub(3,sb) * sub_ha    !dissolved MeHg in lateral flow (mg)
         varoute(40,ihout) = Hg_gwq_sub(1,sb) * sub_ha    !dissolved Hg0 in groundwater return flow  (mg)
         varoute(41,ihout) = Hg_gwq_sub(2,sb) * sub_ha    !dissolved Hg2+ in groundwater return flow (mg)
         varoute(42,ihout) = Hg_gwq_sub(3,sb) * sub_ha    !dissolved MeHg in groundwater return flow (mg)
         varoute(43,ihout) = Hg_sedyld_sub(1,sb) * sub_ha    !particulate Hg0 in sediment yield (mg)
         varoute(44,ihout) = Hg_sedyld_sub(2,sb) * sub_ha    !particulate Hg2+ in sediment yield (mg)
         varoute(45,ihout) = Hg_sedyld_sub(3,sb) * sub_ha    !particulate MeHg in sediment yield (mg)
         varoute(46,ihout) = Hg_wyld_sub(1,sb) * sub_ha     !dissolved Hg0 in water yield (mg)
         varoute(47,ihout) = Hg_wyld_sub(2,sb) * sub_ha    !dissolved Hg2+ in water yield (mg)
         varoute(48,ihout) = Hg_wyld_sub(3,sb) * sub_ha    !dissolved MeHg in water yield (mg)
...
! monthly subbasin mercury values Jaehak 2017
         submono(21,ihout) = submono(21,ihout) + Hg_dry_total_sub(1,2,sb)    !total dissolved Hg2 dry deposition (mg/ha)
         submono(22,ihout) = submono(22,ihout) + Hg_dry_total_sub(2,2,sb)    !total particulate Hg2+ dry deposition  (mg/ha)
         submono(23,ihout) = submono(23,ihout) + Hg_dry_leaf_sub(1,2,sb)    !leaf dissolved Hg2 dry deposition  (mg/ha)
         submono(24,ihout) = submono(24,ihout) + Hg_dry_leaf_sub(2,2,sb)    !DIRECT particulate Hg2+ dry deposition (mg/ha)
         submono(25,ihout) = submono(25,ihout) + Hg_wet_total_sub(1,2,sb)    !! total dissolved Hg2+ wet deposition (mg/ha)
         submono(26,ihout) = submono(26,ihout) + Hg_wet_direct_sub(1,2,sb)    !! direct dissolved Hg2+ wet deposition, (mg/ha)
         submono(27,ihout) = submono(27,ihout) + Hg_wet_thru_sub(1,2,sb)     !throughfall dissolved Hg2+ wet deposition, (mg/ha)
         submono(28,ihout) = submono(28,ihout) + Hg_leaf_fall_sub(1,1,sb)    !leaf fall of dissolved Hg0  (mg/ha)
         submono(29,ihout) = submono(29,ihout) + Hg_leaf_fall_sub(1,2,sb)    !leaf fall of dissolved Hg2+  (mg/ha)
         submono(30,ihout) = submono(30,ihout) + Hg_leaf_fall_sub(2,2,sb)    !leaf fall of particulate Hg2+  (mg/ha)
         submono(31,ihout) = submono(31,ihout) + Hg_leaf_sub(1,1,sb)    !leaf storage of dissolved Hg0  (mg/ha)
         submono(32,ihout) = submono(32,ihout) + Hg_leaf_sub(1,2,sb)    !leaf storage of dissolved Hg+ (mg/ha)
         submono(33,ihout) = submono(33,ihout) + Hg_leaf_sub(2,2,sb)    !leaf storage of particulate Hg2+ (mg/ha)
         submono(34,ihout) = submono(34,ihout) + Hg_residue_sub(1,1,sb)    !residue storage of dissolved Hg0 (mg/ha)
         submono(35,ihout) = submono(35,ihout) + Hg_residue_sub(1,2,sb)    !residue storage of dissolved Hg2+ (mg/ha)
         submono(36,ihout) = submono(36,ihout) + Hg_residue_sub(2,2,sb)    !residue storage of particulate Hg2+ (mg/ha)
         submono(37,ihout) = submono(37,ihout) + Hg_soil_sub(1,1,sb)    !soil storage of dissolved Hg0 (mg/ha)
         submono(38,ihout) = submono(38,ihout) + Hg_soil_sub(1,2,sb)    !soil storage of dissolved Hg2+(mg/ha)
         submono(39,ihout) = submono(39,ihout) + Hg_soil_sub(1,3,sb)    !soil storage of dissolved MeHg (mg/ha)
         submono(40,ihout) = submono(40,ihout) + Hg_soil_sub(2,1,sb)    !soil storage of particulate Hg0  (mg/ha)
         submono(41,ihout) = submono(41,ihout) + Hg_soil_sub(2,2,sb)    !soil storage of particulate Hg2+ (mg/ha)
         submono(42,ihout) = submono(42,ihout) + Hg_soil_sub(2,3,sb)    !soil storage of particulate MeHg (mg/ha)
         submono(43,ihout) = submono(43,ihout) + Hg_shallst_sub(1,1,sb)    !shallow aquifer storage of dissolved Hg0 (mg/ha)
         submono(44,ihout) = submono(44,ihout) + Hg_shallst_sub(1,2,sb)    !shallow aquifer storage of dissolved Hg2+ (mg/ha)
         submono(45,ihout) = submono(45,ihout) + Hg_shallst_sub(1,3,sb)    !shallow aquifer storage of dissolved MeHg (mg/ha)
         submono(46,ihout) = submono(46,ihout) + Hg_surq_sub(1,sb)    !dissolved Hg0 in runoff (mg/ha)
         submono(47,ihout) = submono(47,ihout) + Hg_surq_sub(2,sb)    !dissolved Hg2+ in runoff (mg/ha)
         submono(48,ihout) = submono(48,ihout) + Hg_surq_sub(3,sb)    !dissolved MeHg in runoff (mg/ha)
         submono(49,ihout) = submono(49,ihout) + Hg_latq_sub(1,sb)    !dissolved Hg0 in lateral flow (mg/ha)
         submono(50,ihout) = submono(50,ihout) + Hg_latq_sub(2,sb)    !dissolved Hg2+ in lateral flow (mg/ha)
         submono(51,ihout) = submono(51,ihout) + Hg_latq_sub(3,sb)    !dissolved MeHg in lateral flow (mg/ha)
         submono(52,ihout) = submono(52,ihout) + Hg_perc_sub(1,sb)    !dissolved Hg0 percolated to shallow aquifer  (mg/ha)
         submono(53,ihout) = submono(53,ihout) + Hg_perc_sub(2,sb)    !dissolved Hg2+ percolated to shallow aquifer (mg/ha)
         submono(54,ihout) = submono(54,ihout) + Hg_perc_sub(3,sb)    !dissolved MeHg percolated to shallow aquifer  (mg/ha)
         submono(55,ihout) = submono(55,ihout) + Hg_gwq_sub(1,sb)    !dissolved Hg0 in groundwater return flow  (mg/ha)
         submono(56,ihout) = submono(56,ihout) + Hg_gwq_sub(2,sb)    !dissolved Hg2+ in groundwater return flow (mg/ha)
         submono(57,ihout) = submono(57,ihout) + Hg_gwq_sub(3,sb)    !dissolved MeHg in groundwater return flow (mg/ha)
         submono(58,ihout) = submono(58,ihout) + Hg_sedyld_sub(1,sb)    !particulate Hg0 in sediment yield (mg/ha)
         submono(59,ihout) = submono(59,ihout) + Hg_sedyld_sub(2,sb)    !particulate Hg2+ in sediment yield (mg/ha)
         submono(60,ihout) = submono(60,ihout) + Hg_sedyld_sub(3,sb)    !particulate MeHg in sediment yield (mg/ha)
         submono(61,ihout) = submono(61,ihout) + Hg_wyld_sub(1,sb)    !dissolved Hg0 in water yield (mg/ha)
         submono(62,ihout) = submono(62,ihout) + Hg_wyld_sub(2,sb)    !dissolved Hg2+ in water yield (mg/ha)
         submono(63,ihout) = submono(63,ihout) + Hg_wyld_sub(3,sb)    !dissolved MeHg in water yield (mg/ha)
         submono(64,ihout) = submono(64,ihout) + Hg_sc_sub(sb)    !total Hg conc in soil layer 1 (ng/kg)
         submono(65,ihout) = submono(65,ihout) + Hg0_vola_sub(sb)    !Hg0 volatilization from soil to atmosphere (mg/ha)
