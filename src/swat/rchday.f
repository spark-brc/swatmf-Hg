...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      do j = 1, subtot
        ...
        pdvar(2) = rchdy(2,j)       !!flow out (m^3/s)
 ...
 !! added for NO3 Concentration to output.rch (daily only) gsm 10/26/2011
        ...
        pdvar(47) = rchdy(47,j)     !outflow dissolved Hg0 mg   Mercury Jaehak 2017
        pdvar(48) = rchdy(48,j)     !outflow dissolved Hg2+ mg  
        pdvar(49) = rchdy(49,j)     !outflow dissolved MeHg mg  
        pdvar(50) = rchdy(50,j)     !outflow particulate Hg0 mg 
        pdvar(51) = rchdy(51,j)     !outflow particulate Hg2+ mg 
        pdvar(52) = rchdy(52,j)     !outflow particulate MeHg mg  
        pdvar(53) = rchdy(53,j)     !inflow dissolved Hg0 mg  
        pdvar(54) = rchdy(54,j)     !inflow dissolved Hg2+ mg  
        pdvar(55) = rchdy(55,j)     !inflow dissolved MeHg mg  
        pdvar(56) = rchdy(56,j)     !inflow particulate Hg0 mg 
        pdvar(57) = rchdy(57,j)     !inflow particulate Hg2+ mg 
        pdvar(58) = rchdy(58,j)     !inflow particulate MeHg mg 
        pdvar(59) = rchdy(59,j)     !channel water stored THg mg    
        pdvar(60) = rchdy(60,j)     !channel water MeHg mg     
        pdvar(61) = rchdy(61,j)     !channel sediment1 THg mg      
        pdvar(62) = rchdy(62,j)     !channel sediment1 MeHg mg     
        pdvar(63) = rchdy(63,j)     !THg in water, ppb   channel  sediment2 THg mg    
        pdvar(64) = rchdy(64,j)     !channel sediment1 THg mg/kg      
!!  compute month and day given julian day
     ...
     &             rch_dakm(j), (pdvr(ii), ii = 1, 46)
                write (7746001,5000) j, subgis(j), iida,       
     &             rch_dakm(j),pdvr(2), (pdvr(ii), ii = 47, itotr)
            end if
            
            if(icalen == 1) then
                write (7,5002) j, subgis(j), i_mo, icl(iida),
     &             iyr, rch_dakm(j), (pdvr(ii), ii = 1, 46)
                write (7746001,5002) j, subgis(j), i_mo, icl(iida),
     &             iyr, rch_dakm(j),pdvr(2), (pdvr(ii), ii = 47, itotr)
            end if
...
!!    added for binary files 3/25/09 gsm line below and write (77777
     ...
     &                               (pdvr(ii), ii = 1, 46) 
              write (7746001,6000) j, subgis(j), iida, rch_dakm(j),
     &                    pdvr(2),   (pdvr(ii), ii = 47, itotr) 
          endif
          
          if (icalen == 1) then
              write (7,6002) j, subgis(j), i_mo, icl(iida),
     &          iyr, rch_dakm(j),(pdvr(ii), ii = 1, 46)
              write (7746001,6002) j, subgis(j), i_mo, icl(iida),
     &          iyr, rch_dakm(j),pdvr(2),(pdvr(ii), ii = 47, itotr)
          endif
          endif
  ...
  !  increase to 45 in loops below from 42 gsm 10/26/2011      
     ...
     &                                        (pdvar(ii), ii = 1, 46)
              write(7746001,5000) j, subgis(j), iida, rch_dakm(j),
     &                            pdvar(2),  (pdvar(ii), ii = 47, itotr)
          endif
          
          if (icalen == 1) then
              write (7,5002) j, subgis(j), i_mo, icl(iida),
     &            iyr, rch_dakm(j),(pdvar(ii), ii = 1, 46)
              write (7746001,5002) j, subgis(j), i_mo, icl(iida),
     &            iyr, rch_dakm(j),pdvar(2),(pdvar(ii), ii = 47, itotr)
          endif
...
!!    added for binary files 3/25/09 gsm line below and write (77777
 ...
 5000 format ('REACH ',i4,1x,i8,1x,i5,100e12.4)
 5001 format ('REACH ',i4,1x,i8,1x,i5,1x,i5,100e12.4)
 5002 format ('REACH ',i4,1x,i8,2x,i2,1x,i2,1x,i4,1x,100e12.4)
 6000 format ('REACH ',i4,1x,i8,1x,i5,100e12.4)
 6002 format ('REACH ',i4,1x,i8,1x,i2,1x,i2,1x,i4,1x,100e12.4)
      end