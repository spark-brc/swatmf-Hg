      ...
      real, dimension (:), allocatable :: etremain,deg_z
      real, dimension (:), allocatable :: rttlc,sed_depth1,sed_depth2
      ...
      real, dimension (:), allocatable :: rchrg_hg_conc

      integer icalen
      real :: prf_bsn,hg_sedcon
	real :: hg_soil_coeff, hg_ch_coeff, hg_landfill_coeff, hg_atmo_co
      
      real, dimension (:), allocatable :: alph_e,qhg,dep_z
...
!! septic variables for output.std
      ...
      real :: fertorgp, sed_deposit,sed_dep_rto
...
!! septic change added iseptic 1/28/09 gsm
      ...
      real, dimension (:), allocatable :: sptqs,percp,perchg
...
!  added 1/27/09 when making septic changes
      ...
      real, dimension (:,:), allocatable :: sol_rsd,sol_fop,sol_rockp
...
! mhru = maximum number of hydrologic response units
      ...
      real, dimension (:), allocatable :: irrig_yr_sw,irrig_yr_gw !rtb
      real, dimension (:), allocatable :: hru_et_yr !rtb
      integer :: irrig_switch !rtb
...
!    Drainmod tile equations  01/2006
      ...
      real, dimension (:), allocatable :: rchrg_p,rchrg_hg !rtb modflow
...
!!    increased hedr(42) to hedr(45) for output.rch gsm 10/17/2011
      character(len=13) ::  heds(127),hedb(72),hedr(64),hedrsv(60)
...
!! By Zhang for C/N cycling
      ...
      !rtb riyana
      logical :: nit_inputs
      integer :: in_nit_inputs,in_index,in_hru,num_inputs
      real, dimension(:,:), allocatable :: nit_mass

!! Mercury variables definition by Jaehak Jeong 2016
      !Type of mucury includes 1=Hg0, 2=Hg2+, 3=MeHg
      
   !! HRU Mercury pools
      real*8, dimension(:), allocatable :: frac_groundcover  !Fraction of ground cover by vegetation
      real*8, dimension(:), allocatable :: leaffall, biom_lf  !Biomass lost in leaf fall (kg/ha)

      !dimension(d/p,iHg,ihru) d(=1):dissolved, p(=2):particulate, iHg(=1):Hg0, iHg(=2):Hg(II), iHg(=3):MeHg
      real*8, dimension(:,:,:), allocatable :: Hg_airconc  ! Hg concentration in the air (ppm) in HRUs
      real*8, dimension(:,:,:), allocatable :: Hg_pcpconc  ! Hg concentration in the rainfall (ppm) in HRUs
      real*8, dimension(:,:,:), allocatable :: Hg_airconc_sub  ! Input Hg concentration in the air (ppm) by subbasin
      real*8, dimension(:,:,:), allocatable :: Hg_pcpconc_sub  ! Input Hg concentration in the rainfall (ppm) by subbasin
      real*8, dimension(:,:,:), allocatable :: Hg_dry_total  !Dry deposit of dissolved Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_dry_leaf  !Leaf Dry deposit of Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_dry_land  !Direct Dry deposit of Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_total  !Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_direct  !Direct Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_thru  !ThroughFall Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_snow  !Hg in snow deposit (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_smelt !Hg in snow melt flow (runoff and infiltration) (mg/ha)
      real*8, dimension(:,:,:,:), allocatable :: Hg_leaf  ! Hg on plant canopy (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_leaf_added  !Hg added to canopy with rainfall (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_leaf_fall  ! Hg in leaf/litter fall (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_residue   !Hg in surface residue (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_recharge !Hg shallow aquifer recharge (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_shallst !Hg shallow aquifer storage (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_soil  !dissolved fraction Hg in the soil layer (mg/ha)
      real*8, dimension(:), allocatable :: Hg0_vola,Hg0_vola_sub  ! Hg0 volatilization from soil to atmosphere (mg/ha)
      real*8, dimension(:), allocatable :: Hg_sc,Hg_sc_sub  !total Hg concentration in the soil layer (ng THg/kg Soil)
      !dimension(d/p,iHg,ihru,ilyr)
      real*8, dimension(:,:,:,:), allocatable :: Hg_soil_layer  !dissolved fraction Hg in the soil layer (mg/ha)

      !dimension(d/p,iHg,ihru)
      real*8, dimension(:,:), allocatable :: Hg_wyld    !Dissolved Hg in water yield (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_surq    !Dissolved Hg in surface runoff (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_sedyld  !Particulate Hg lost by soil erosion (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_latq    !Dissolved Hg in lateral flow (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_gwq     !Dissolved Hg in groundwater return flow (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_perc    !Dissolved Hg percolating into shallow aquifer (mg/ha)

   !! Channel/reservoir 
      !dimension(d/p,iHg,irte)
      real*8, dimension(:,:,:,:), allocatable :: Hg_chstor !solid fraction Hg existing in channels (mg)
      real*8, dimension(:,:,:), allocatable :: Hg_chq !solid fraction Hg transported channel outlet (mg/d)
      real*8, dimension(:,:,:,:), allocatable :: Hg_resstor ! Hg existing in reservoirs (mg)
      real*8, dimension(:,:,:), allocatable :: Hg_resq !solid fraction Hg discharging at reservoir outlet (mg/d)

      real :: frac_snomelt, frac_leaffall,hgsurqco,canstoday,hgpercoa
      real, dimension(:), allocatable :: rsdinit
      
      !Jaehak error handling
      real:: fr_curb,irelease,hgpercco,hg_sol_co

      !dimension(d/p,iHg,isub) d(=1):dissolved, p(=2):particulate, iHg(=1):Hg0, iHg(=2):Hg(II), iHg(=3):MeHg
      real*8, dimension(:,:,:), allocatable :: Hg_dry_total_sub  !Dry deposit of dissolved Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_dry_leaf_sub  !Leaf Dry deposit of Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_dry_land_sub  !Direct Dry deposit of Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_total_sub  !Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_direct_sub  !Direct Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_wet_thru_sub  !ThroughFall Wet deposit Hg (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_leaf_sub  !washable fraction of Hg on plant canopy (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_leaf_fall_sub  ! Hg in leaf/litter fall (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_residue_sub   !Hg in surface residue (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_soil_sub  !dissolved fraction Hg in the soil layer (mg/ha)
      real*8, dimension(:,:,:), allocatable :: Hg_shallst_sub !Hg shallow aquifer storage (mg/ha)
      !dimension(iHg,isub) iHg(=1):Hg0, iHg(=2):Hg(II), iHg(=3):MeHg
      real*8, dimension(:,:), allocatable :: Hg_surq_sub    !Dissolved Hg in surface runoff (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_sedyld_sub  !Particulate Hg lost by soil erosion (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_latq_sub    !Dissolved Hg in lateral flow (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_gwq_sub     !Dissolved Hg in groundwater return flow (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_perc_sub    !Dissolved Hg percolating into shallow aquifer (mg/ha)
      real*8, dimension(:,:), allocatable :: Hg_wyld_sub    !Dissolved Hg in water yield (mg/ha)
      real*8, dimension(:), allocatable :: res_sed_init    !initial sediment in reservoiir
      real*8, dimension(:), allocatable :: sed_wt    !initial sediment weight in reach (tons)
	real*8::conc_hgpsed1(3),conc_hgpsed2(3),conc_hgdsed1(3),conc_hgdsed2(3)
