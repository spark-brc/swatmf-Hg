...
!! initialize variables    
      mvaro = 52 !Mercury output added, Jaehak 2017
      mhruo = 124
      mrcho = 100
...
!! arrays for reservoir output
      allocate (icolrsv(41))
      allocate (resoutm(59,mres))
      allocate (resouta(59,mres))
      allocate (resouty(59,mres))
...
!!    arrays which contain data related to HRUs
      ...
      allocate (aird(mhru))
      allocate (irrig_yr_sw(mhru)) !rtb irrigation
      allocate (irrig_yr_gw(mhru)) !rtb irrigation
      allocate (hru_et_yr(mhru)) !rtb
...
!    Drainmod tile equations  01/2006 
      ...
      allocate (rchrg_hg(mhru))    !amount of Hg getting to the shallow aquifer (rtb modflow)
      allocate (re(mhru))
...
!!    arrays which contain data related to HRU output 
      ...
      allocate (hrumono(120,mhru))
      allocate (hruyro(120,mhru))
      allocate (hruaao(120,mhru))
...
!! Variables for soil P and additional operations mjw
	...
	allocate(rspc_a(mhru))	
      ...
      !! Mercury parameters Jaehak Jeong 2016
      !! =====================================
      allocate(Hg_airconc(2,3,mhru),Hg_airconc_sub(2,3,msub),
     & Hg_snow(2,3,mhru),Hg_leaf(2,3,mhru,2),Hg_sc(mhru),
     & Hg_sc_sub(msub),Hg_residue(2,3,mhru),
     & Hg_dry_total(2,3,mhru),Hg_dry_land(2,3,mhru),
     & Hg_wet_thru(2,3,mhru),Hg_wet_direct(2,3,mhru),
     & Hg_wet_total(2,3,mhru),Hg_pcpconc_sub(2,3,msub),
     & Hg_dry_leaf(2,3,mhru),Hg_wet_leaf_added(2,3,mhru),
     & Hg_soil(2,3,mhru),Hg_shallst(2,3,mhru),Hg0_vola(mhru),
     & Hg0_vola_sub(msub),Hg_soil_layer(2,3,mhru,mlyr)) 
      allocate(Hg_leaf_fall(2,3,mhru),
     & Hg_wyld(3,mhru),Hg_surq(3,mhru),Hg_latq(3,mhru),
     & Hg_gwq(3,mhru),Hg_perc(3,mhru),Hg_sedyld(3,mhru),
     & Hg_smelt(2,3,mhru),Hg_recharge(2,3,mhru),
     & frac_groundcover(mhru),rsdinit(mhru),leaffall(mhru),
     & biom_lf(mhru))

      allocate(Hg_chstor(2,3,mch,3),Hg_chq(2,3,mch))
      allocate(Hg_resstor(2,3,mres,3),Hg_resq(2,3,mres))
      allocate(Hg_dry_total_sub(2,3,msub),Hg_dry_leaf_sub(2,3,msub),
     & Hg_dry_land_sub(2,3,msub),Hg_wet_total_sub(2,3,msub),
     & Hg_wet_direct_sub(2,3,msub),Hg_wet_thru_sub(2,3,msub),
     & Hg_leaf_sub(2,3,msub),Hg_leaf_fall_sub(2,3,msub),
     & Hg_residue_sub(2,3,msub),Hg_soil_sub(2,3,msub),
     & Hg_shallst_sub(2,3,msub))
      allocate(Hg_surq_sub(3,msub), Hg_sedyld_sub(3,msub), 
     & Hg_latq_sub(3,msub),Hg_gwq_sub(3,msub),Hg_perc_sub(3,msub),
     & Hg_wyld_sub(3,msub),res_sed_init(mres),sed_wt(msub),qhg(msub),
     & deg_z(2),sed_depth1(msub),sed_depth2(msub),
     & dep_z(2))
      call zero0
      call zero1
      call zero2
