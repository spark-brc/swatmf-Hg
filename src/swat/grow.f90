...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	  ...
	  real :: laimax, rto,lai0, leafb,leafk,leafbiomass
      integer::calyr(40),phucon(40),phudec(40)
      calyr = (/1983,1984,1985,1986,1987,1988,1989,1990,1991,1992,1993,1994,1995,1996,1997,1998,1999,2000,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022/)
      phucon = (/5246,5105,5109,4945,5140,5147,5296,5518,5259,5174,5008,5553,5093,5092,5375,5670,5392,5191,5320,5273,5157,5453,5247,5399,5534,5569,5582,5430,5349,5301,5454,5455,5569,5582,5430,5349,5301,5454,5455,5455/)
      phudec = (/2454,2403,2393,2163,2305,2367,2332,2517,2350,2263,2186,2669,2308,2322,2438,2640,2458,2363,2504,2346,2248,2481,2558,2436,2476,2664,2616,2556,2556,2556,2636,2522,2664,2616,2556,2556,2556,2636,2522,2522/)
      ...
      rto = 1.
        ...
        idp = idplt(j)
        do ii=1,32
            if(calyr(ii)==iyr) then
                if(idp ==8) then
                    phu_plt(j) = phucon(ii)
                elseif(idp==6.or.idp==7) then
                    phu_plt(j) = phudec(ii)
                endif
            endif
        end do
	    ...
	    if (reg > 0.) then
          ...
          if (phuacc(j) <= dlai(idp)) then
            ...
            if (laiday(j) > lai_yrmx(j)) lai_yrmx(j) = laiday(j)
			  if (idc(idp) == 7) then
				if (idp==6.or.idp==8) then !mixed forest  Baskerville, 1972 (and evergreen)
				  leafb = 0.795  !Xu et al. (2014) for Korea
				  leafk = -1.409
				elseif (idp==7) then !deciduous  
				  leafb = 0.964  !Xu et al. (2014) for Korea
				  leafk = -1.376
				endif
				

                biom_lf(j) = 10 ** (leafb * log10(bio_ms(j)) + leafk) 
              else
                biom_lf(j) = bio_ms(j)  
              endif
			  
		  else !leaf fall based on DLAI modified by Jaehak 2017 for leaf biomass update
			lai0 = laiday(j)
			laiday(j) = olai(j) * (1. - phuacc(j)) / &                   
	                                (1. - dlai(idp))
			deltalai = lai0 - laiday(j) 
			if (deltalai>0) then
				leaffall(j) = deltalai/ olai(j) * biom_lf(j) ! leaf fall (kg/ha) for Mercury Jaehak 2017
				biom_lf(j) = biom_lf(j) - leaffall(j)
				bio_ms(j) = bio_ms(j) - leaffall(j) !kg/ha
				sol_rsd(1,j) = sol_rsd(1,j) + leaffall(j)  !kg/ha
		   end if
		  end if
